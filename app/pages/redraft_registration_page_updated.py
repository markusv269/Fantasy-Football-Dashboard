...
def _season_status() -> rx.Component:
    return rx.card(
        rx.hstack(
            rx.icon("clipboard-check", size=24, color="#10B981"),
            rx.vstack(
                rx.text(
                    "Ligen erstellt – Nachrückerliste geöffnet",
                    size="3",
                    weight="bold",
                    class_name=TEXT_PRIMARY,
                ),
                rx.text(
                    "85 Ligen · 1.020 Spielerplätze vergeben · ",
                    rx.text(
                        RedraftRegistrationState.waitlist_count.to_string(),
                        weight="bold",
                    ),
                    " Nachrücker aktuell auf der Warteliste.",
                    size="2",
            ...
            rx.spacer(),
            rx.badge("Nachrücker", color_scheme="amber", variant="soft", size="2"),
            spacing="3",
            align="center",
            width="100%",
        ),
        size="3",
        width="100%",
        class_name="border-l-4 border-l-emerald-500 bg-emerald-500/5",
    )
...
def _success_card() -> rx.Component:
    return rx.card(
        rx.vstack(
            ...
            rx.heading("Als Nachrücker registriert!", size="5", weight="bold"),
            rx.text(
                "Dein Änderungscode:",
                size="2",
                class_name=TEXT_SECONDARY,
            ...
            rx.text(
                "Deine Anmeldung steht jetzt auf der Nachrückerliste. "
                "Bewahre den Code gut auf – ohne ihn kannst du deine "
...
def _form() -> rx.Component:
    return rx.card(
        rx.vstack(
            rx.heading("Als Nachrücker anmelden", size="5", weight="bold"),
            rx.divider(),
            _form_group(
                "Sleeper Name",
                True,
            ...
            rx.button(
                rx.cond(
                    ...
                    rx.spinner(size="2"),
                    rx.text("Auf Nachrückerliste setzen"),
                ),
...
def _stats_bar() -> rx.Component:
    return rx.grid(
        _stat_tile(
            "Ligen",
            RedraftRegistrationState.fixed_league_count.to_string(),
            "trophy",
            "#10B981",
            "bereits erstellt",
        ),
        _stat_tile(
            "Spieler in Ligen",
            RedraftRegistrationState.players_in_leagues.to_string(),
            "users",
            "#10B981",
            "85 × 12 Plätze",
        ),
...
def _waitlist_entry_row(e: dict, index: int) -> rx.Component:
    return rx.table.row(
        rx.table.cell(
            rx.text((index + 1).to_string(), size="2", weight="bold", class_name=TEXT_SECONDARY)
        ),
        rx.table.cell(
            rx.text(e["sleeper"].to(str), size="2", weight="bold", class_name=TEXT_PRIMARY)
        ),
        rx.table.cell(
            rx.text(e["discord"].to(str), size="2", class_name=TEXT_SECONDARY)
        ),
        rx.table.cell(
            rx.cond(
                e["commish"].to(bool),
                rx.badge(rx.icon("crown", size=12), "Ja", color_scheme="red", variant="solid", size="1"),
                rx.badge("Nein", color_scheme="gray", variant="soft", size="1"),
            )
        ),
        rx.table.cell(
            rx.text(e["mates_display"].to(str), size="2", class_name=TEXT_PRIMARY)
        ),
        rx.table.cell(
            rx.badge(e["mutual_count"].to(str), color_scheme="green", variant="soft", size="1")
        ),
        rx.table.cell(
            rx.text(e["created_display"].to(str), size="1", class_name=TEXT_SECONDARY)
        ),
    )
    ...
    return rx.card(
        rx.vstack(
            rx.hstack(
                ...
                rx.heading("Aktuelle Nachrückerliste", size="5", weight="bold"),
                rx.badge(
                    RedraftRegistrationState.waitlist_count.to_string(),
                    color_scheme="amber",
                    variant="soft",
            ...
            rx.text(
                "Die ersten 1.020 Anmeldungen sind den 85 bereits erstellten "
                "Ligen zugeordnet. Alle weiteren Anmeldungen werden hier in "
                "der Reihenfolge ihres Eingangs als Nachrücker angezeigt.",
                size="2",
                class_name=TEXT_SECONDARY,
            ),
            _stats_bar(),
            rx.text(
                RedraftRegistrationState.league_status_text,
                size="1",
                class_name="italic " + TEXT_SECONDARY,
            ),
            rx.cond(
                RedraftRegistrationState.is_loading,
                rx.center(rx.spinner(size="3"), padding_y="40px", width="100%"),
                rx.cond(
                    RedraftRegistrationState.waitlist_entries.length() > 0,
                    rx.box(
                        rx.table.root(
                            rx.table.header(
                                rx.table.row(
                                    ...
                                    rx.table.column_header_cell("Mitspieler-Wünsche"),
                            ...
                            rx.table.body(
                                rx.foreach(
                                    RedraftRegistrationState.waitlist_entries,
                                    _waitlist_entry_row,
                                ),
                            ),
                            variant="surface",
                    ...
                    rx.vstack(
                        ...
                        rx.text(
                            "Noch keine Nachrücker vorhanden.",
                            size="2",
                            weight="medium",
                            class_name=TEXT_PRIMARY,
                        ),
                        rx.text(
                            "Neue Anmeldungen werden automatisch hier einsortiert.",
                            size="1",
                            class_name=TEXT_SECONDARY,
                        ),
                        spacing="2",
